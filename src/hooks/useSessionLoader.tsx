...
const useSessionLoader = () => {
  ...
  const getSessions = useCallback(
    ...
    [selectedEndpoint, setSessionsData, setIsSessionsLoading, userId]
  );
  ...
  const getSession = useCallback(
    async (sessionId: string, agentId: string) => {
      ...
      try {
        ...
        // 直接清空当前消息，确保新的会话消息能够显示
        setMessages([]);
        
        // 检查response中是否有runs字段
        let sessionHistory;
        if (response.runs) {
          sessionHistory = response.runs;
        } else if (response && response.memory && response.memory.runs) {
          sessionHistory = response.memory.runs;
        } else {
          return null;
        }
